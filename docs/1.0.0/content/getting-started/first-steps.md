...
### Step 4: Use a Config File (Alternative)

Create a config file so you don't need to repeat flags:

```bash
npx @fsegurai/manifest-generator --init
```

This creates `manifest-generator.config.ts` with sensible defaults plus a sample `docs/` folder. Then just run:
...
Fields with empty/falsy values (`null`, `false`, `''`, `undefined`) are stripped by `cleanItem()` before writing — that's
why a page with no `tags` set simply omits the field instead of showing `"tags": []`.
...
[
  {
    "label": "README",
    "route": "README",
    "breadcrumb": [{ "label": "README", "route": "README" }]
  },
  {
    "label": "Getting Started Guide",
    "route": "getting-started",
    "tags": ["guide", "beginner"],
    "breadcrumb": [{ "label": "Getting Started Guide", "route": "getting-started" }]
  },
  {
    "label": "Reference",
    "route": "api/reference",
    "breadcrumb": [
      { "label": "Api", "route": "api" },
      { "label": "Reference", "route": "api/reference" }
    ]
  }
]
```
...
```bash
npx @fsegurai/manifest-generator --config ./config/manifest-generator.config.ts
```

### Scaffold a New Project
```bash
npx @fsegurai/manifest-generator --init ./new-docs-project
```
...
2. Explore [Configuration](../guides/configuration.md) for customization
...
npx @fsegurai/manifest-generator --dry-run --all    # Preview without writing
