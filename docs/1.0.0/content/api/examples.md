...
const results = generateManifestsWithDiscovery('./workspace', {
  docsSubfolder: 'docs',
  dryRun: true, // logs what would be written; nothing is written to disk
});
```

### Reading Your Own Config File

`loadConfig`/`loadConfigFromFile`/`mergeConfig` are CLI-internal (in `src/config.ts`) and not part of the public
package API. If your own script needs to read a `manifest-generator.config.json`, read and parse it directly:

```javascript
import fs from 'fs';
import { generateManifestsWithDiscovery } from '@fsegurai/manifest-generator';

const config = JSON.parse(fs.readFileSync('./manifest-generator.config.json', 'utf8'));

const results = generateManifestsWithDiscovery('./', {
  docsSubfolder: config.docsSubfolder ?? 'docs',
...
// Type-safe config definition — use in manifest-generator.config.ts
export default defineConfig({
  docsSubfolder: 'docs',
  outputDir: './public/manifests',
  exclude: ['drafts/**'],
});
```
