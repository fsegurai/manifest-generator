...
description: External-user documentation for @fsegurai/manifest-generator 1.0.0-beta.3.
...
This `1.0.0-beta.3` docs set reflects:

- **Config file**: `manifest-generator.config.json/.js/.ts` with `--config`, `defineConfig()`, and CLI-flag-over-config priority
- **--init scaffold**: one-command setup that creates a config file and sample docs structure
- **Extended validation**: 11 new frontmatter checks — missing label, future dates, unknown keys, invalid types, and more
- **js-yaml parser**: replaced the custom parser with `js-yaml` (JSON_SCHEMA) for robust YAML block scalar and array support
- **ALLOWED_FRONTMATTER_KEYS**: public API export for external validation tooling
- **104 tests**: expanded coverage across config loading, validation, CLI integration, and edge cases
