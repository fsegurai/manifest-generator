...
Use `--recursive` (or `recursive: true` in the API/config file) to discover projects more than one level deep. Without it, only `standalone` is found.
...
The config file is auto-detected when running from the project root.

## `--init` scaffold output

Running `npx @fsegurai/manifest-generator --init` creates:
