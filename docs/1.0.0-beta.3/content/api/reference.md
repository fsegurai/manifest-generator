...
```ts
const result = generateManifest('./docs/1.0.0-beta.3/content');
...
Type-safe helper for config files. Use in `manifest-generator.config.ts`:
...
### `loadConfig(configPath: string): ConfigFile | null`

Loads a config file from a `.ts`, `.js`, or `.json` path. Used internally by the CLI.

```ts
const config = loadConfig('./manifest-generator.config.json');
if (config) {
...
A readonly array of all recognised frontmatter key names. Useful for external validation tooling:

```ts
import { ALLOWED_FRONTMATTER_KEYS } from '@fsegurai/manifest-generator';

console.log(ALLOWED_FRONTMATTER_KEYS);
// => ['label', 'description', 'tags', 'keywords', 'order', 'isTitle', 'isParent', 'icon', ...]
```
...
- Config files are loaded from `.ts`, `.js`, or `.json` format.
